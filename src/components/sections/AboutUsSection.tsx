...
export default function AboutUsSection() {
  ...
  return (
    <section className="w-full bg-white py-16 md:py-24 border-t border-zinc-100 overflow-hidden">
      <div className="w-full max-w-[1640px] mx-auto px-6 md:px-12">
        ...
        <div className="mb-12 md:mb-16 flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          <div className="flex items-center gap-3">
            <span className="text-[13px] font-medium text-zinc-500 mr-2 hidden sm:block">
              Śledź nas:
            </span>
            ...
            <a 
              ...
              target="_blank" 
              rel="noreferrer" 
              className="w-11 h-11 rounded-full bg-white border border-zinc-200 flex items-center justify-center text-zinc-500 hover:text-zinc-900 hover:border-zinc-400 hover:shadow-sm transition-all duration-300 group"
            ...
            <a 
              ...
              target="_blank" 
              rel="noreferrer" 
              className="w-11 h-11 rounded-full bg-white border border-zinc-200 flex items-center justify-center text-zinc-500 hover:text-blue-600 hover:border-blue-200 hover:bg-blue-50 transition-all duration-300 group"
              aria-label="Facebook"
            >
            ...
            <a 
              ...
              target="_blank" 
              rel="noreferrer" 
              className="w-11 h-11 rounded-full bg-white border border-zinc-200 flex items-center justify-center text-zinc-500 hover:text-red-600 hover:border-red-200 hover:bg-red-50 transition-all duration-300 group"
              aria-label="YouTube"
            >
        ...
        <div className="flex flex-col lg:flex-row gap-8 xl:gap-12">
          ...
          {/* LEWA STRONA: Odtwarzacz YT ze smaczkami platformy */}
          <div className="w-full lg:w-[60%] flex flex-col">
            ...
            {/* Premium Meta Row - Zweryfikowany profil autorski w stylu YouTube */}
            <div className="mt-5 flex items-center justify-between border-b border-zinc-100 pb-5">
              <div className="flex items-center gap-3.5">
                ...
                <div>
                  <div className="flex items-center gap-1.5">
                    ...
                    {/* Precyzyjny, niebieski znaczek weryfikacji SVG */}
                  ...
                  <p className="text-[12px] text-zinc-400 font-medium mt-1">Pokaż się online z dobrej storny!</p>
                </div>
              </div>
            </div>
          </div>

          {/* PRAWA STRONA: Mapa 3D z precyzyjną pozycją Białegostoku */}
          <div className="w-full lg:w-[40%] flex flex-col justify-between bg-zinc-50 rounded-[24px] p-8 border border-zinc-100 relative overflow-hidden shadow-[inset_0_1px_2px_rgba(0,0,0,0.02)]">
            
            {/* Zaawansowane środowisko projekcji 3D */}
            <div 
              className="w-full relative flex items-center justify-center h-full overflow-hidden pointer-events-none"
              style={{ perspective: '1200px' }}
            >
              {/* Naturalne wygaszanie krawędzi mapy */}
                <div className="absolute inset-x-0 bottom-0 h-8 bg-gradient-to-t from-zinc-50 to-transparent z-10" />              
              <svg 
                viewBox={world.viewBox} 
                // KLUCZOWE: Robimy mapę fizycznie ogromną (350% szerokości), żeby była ostra jak brzytwa.
                className="absolute w-[380%] md:w-[390%] max-w-none h-auto" 
                aria-label={world.label}
                // Dodaje precyzję wektorom:
                shapeRendering="geometricPrecision"
                style={{
                  // USUNIĘTE scale(). Zamiast tego obracamy i przesuwamy (translate) całą wielką mapę, żeby wyśrodkować Polskę.
              ...
              >
                ...
                {/* --- PULSUJĄCY PUNKT GEOGRAFICZNY (BIAŁYSTOK) --- */}
            ...
            {/* Elitarne, wyważone copy biznesowe */}
            <div className="relative z-10 mt-4">
              ...
              <p className="text-[14px] text-zinc-600 leading-relaxed font-normal">
                Fizycznie pracujemy z Białegostoku, ale nasze rozwiązania nie znają granic. Tworzymy strony, aplikacje i systemy dla firm z całej Polski i zagranicy, gwarantując ten sam najwyższy standard niezależnie od odległości.
              </p>
            </div>
          </div>
        </div>

        {/* --- DOLNA CZĘŚĆ: ARCHITEKTURA ZESPOŁU (Siatka Premium) --- */}
        <div className="pt-12">
          <div className="mb-10">
            <h3 className="text-[24px] font-bold text-zinc-950 tracking-tight mb-2">Nasz zespół</h3>
                <p className="text-[14px] text-zinc-500 max-w-[540px] leading-relaxed font-normal">
Kompaktowy zespół to szybsze decyzje i czystsza komunikacja. Poznaj ekspertów, z którymi będziesz współpracować bezpośrednio przy realizacji swojego systemu.
          ...
          {/* Perfekcyjny, zwarty grid. Całkowity brak efektu grayscale – czyste, pełne kolory od samego początku */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {teamProfiles.map((profile) => (
              <div key={profile.name} className="flex flex-col p-6 rounded-[20px] bg-white border border-zinc-100/80 shadow-[0_2px_8px_rgba(0,0,0,0.01)] hover:shadow-[0_20px_40px_rgba(0,0,0,0.04)] hover:border-zinc-200 transition-all duration-300 group">
                
                {/* Profilówka w pełnym kolorze bez żadnych filtrów szarości */}
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[13px] text-zinc-500 leading-relaxed mb-5 font-normal">
                    {profile.description}
                  </p>
                </div>
