...
import { Bot, ArrowLeft, Send, Sparkles, Check, Calendar as CalendarIcon, Clock, ChevronRight, X, HelpCircle, PhoneCall, Scissors, Brush } from 'lucide-react';
...
export default function AssistantDemo({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className={`w-full flex flex-col h-[850px] transition-colors duration-1000 border border-white/10 rounded-3xl shadow-2xl overflow-hidden relative bg-gradient-to-b ${theme.bg}`}>
      ...
      <div className="px-6 py-5 border-b border-white/5 flex items-center bg-white/[0.01] z-20 backdrop-blur-sm">
        <button onClick={onClose} className="flex items-center gap-2 px-3 py-2 -ml-2 rounded-xl text-white/60 hover:text-white hover:bg-white/10 transition-all cursor-pointer">
          <ArrowLeft className="w-5 h-5" /> <span className="font-semibold text-sm">Zakończ demo</span>
        </button>
        <div className="ml-auto flex items-center gap-2 px-3 py-1.5 bg-green-500/10 border border-green-500/20 rounded-full">
          ...
          <span className="text-[11px] font-bold text-green-400 uppercase tracking-widest">Aktywny</span>
        </div>
      </div>
